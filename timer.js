...
async function scheduleTimer() {
  ...
  //   titleText: "周末有课111？", // 标题内容，字体比较大，超过10个字不给显示的喔，也可以不传就不显示
  ...
  const startSemester = await AISchedulePrompt({
    ...
    defaultText: getCurrentDate(), // 文字输入框的默认内容，不传会显示版本号，所以空内容要传个''
    validator: (value) => {
      // 校验函数，如果结果不符预期就返回字符串，会显示在屏幕上，符合就返回false
      console.log(value);
  ...
  function getCurrentDate() {
    ...
    const isBeforeJune15 =
      currentDate.getMonth() < 5 ||
      (currentDate.getMonth() === 5 && currentDate.getDate() < 15);

    let startDate, endDate;

    if (isBeforeJune15) {
      startDate = new Date(year, 1, 27); // 2月27日
      endDate = new Date(year, 2, 5); // 3月5日
    } else {
      startDate = new Date(year, 7, 30); // 8月30日
      endDate = new Date(year, 8, 5); // 9月5日
    }
    ...
    return `${year}-${formatNumber(startDate.getMonth() + 1)}-${formatNumber(
      startDate.getDate()
    )}`;
  }
